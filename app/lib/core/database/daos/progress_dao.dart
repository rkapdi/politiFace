...
class ProgressDao extends DatabaseAccessor<AppDatabase> with _$ProgressDaoMixin {
  ...
  Future<UserNodeProgressEntry?> forNode(String nodeId) => (select(userNodeProgress)..where((p) => p.nodeId.equals(nodeId)))
        .getSingleOrNull();

  Future<List<UserNodeProgressEntry>> all() => select(userNodeProgress).get();

  Future<void> upsert(UserNodeProgressCompanion entry) => into(userNodeProgress).insertOnConflictUpdate(entry);
  ...
  Future<void> insertIfAbsent(UserNodeProgressCompanion entry) => into(userNodeProgress)
