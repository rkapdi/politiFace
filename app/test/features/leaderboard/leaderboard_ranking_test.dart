...
void main() {
  test('rankEntries sorts descending and applies competition ranking', () {
    final entries = rankEntries([
      (userId: 'u1', handle: 'a', score: 10),
      (userId: 'u2', handle: 'b', score: 30),
      (userId: 'u3', handle: 'c', score: 20),
      (userId: 'u4', handle: 'd', score: 20),
      (userId: 'u5', handle: 'e', score: 5),
    ]);

    expect(entries.map((e) => e.userId).toList(),
        ['u2', 'u3', 'u4', 'u1', 'u5'],);
  ...
  test('rankEntries handles empty and single rows', () {
    ...
    final one = rankEntries([(userId: 'u', handle: 'h', score: 0)]);
    expect(one.single.rank, 1);
  });

  test('all tied means everyone is rank 1', () {
    final entries = rankEntries([
      (userId: 'u1', handle: 'a', score: 7),
      (userId: 'u2', handle: 'b', score: 7),
      (userId: 'u3', handle: 'c', score: 7),
    ]);
